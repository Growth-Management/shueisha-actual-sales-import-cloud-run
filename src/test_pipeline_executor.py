...
def test_execute_pipeline_builds_manifest_and_validation_results():
    ...
    uploaded_text = storage_client.uploads[0]["data"].decode("utf-8")
    assert uploaded_text.splitlines()[0].startswith("sales_yyyymm,start_date,end_date,upc,isrc_isbn,vendor_identifier")
    assert uploaded_text.splitlines()[1].startswith("202606,2026-03-29,2026-05-02,,,standard_1month_ios,27")
    ...
    assert payload["staging"]["landing_uploads"][0]["normalized_columns"][:6] == [
        "sales_yyyymm",
        "start_date",
        "end_date",
        "upc",
    ...
    assert payload["staging"]["landing_uploads"][0]["normalized_row_count"] == 1
